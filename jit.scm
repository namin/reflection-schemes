...
(define (ABC-of on ast)
  (cond ((eq? ':consequent on)
         (list (get (get (get (get cs ':children) '(:exp . 0)) ':children) ':exp)
               (get (get (get (get (get (get cs ':children) '(:exp . 1)) ':children) ':alternative) ':children) '(:exp . 1))
               (get (get (get (get cs ':children) '(:exp . 1)) ':children) ':condition)))
        ((eq? ':alternative on)
         (list (get (get (get (get (get (get cs ':children) '(:exp . 1)) ':children) ':consequent) ':children) '(:exp . 1))
               (get (get (get (get cs ':children) '(:exp . 0)) ':children) ':exp)
               (get (get (get (get cs ':children) '(:exp . 1)) ':children) ':condition)))
        (else (list
